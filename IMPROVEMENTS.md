...
- [x] **7.1 — Build blocking local CI.** Run on PRs and pushes to `main`. Jobs: static/offline checks and Chromium mock UI tests on Python 3.12/Ubuntu. Install only Chromium for the primary UI job, including Linux system dependencies. Use Docker mock serving here so that path is exercised; verify health before pytest and pass explicit mock origin for fixture reuse. Cache dependency downloads keyed by the lock, keep browser installs aligned with Playwright. Use current compatible action versions verified from their official repositories at implementation time; pin immutable SHAs if repository policy requires them. Set `permissions: contents: read`, concurrency cancellation for obsolete PR runs, and bounded job timeouts. **Done when:** a deliberate test failure fails the job, logs/containers are collected/cleaned with `always()`, and there is no `continue-on-error` on required checks.
- [x] **7.2 — Preserve reports on success and failure.** Upload reports, browser evidence when present, and server diagnostics using the exact Phase 6 directories. Create a job summary with target, commit, browser, seed, passed/failed/skipped/deselected counts and artifact links. Use artifact names unique per matrix cell, 14-day retention by default, and fail/report missing expected JUnit output rather than silently announcing success. **Done when:** inspect artifacts from one passing and one deliberately failing PR run and confirm counts/paths.
...
- [x] Decisions affecting implemented scope are recorded; permission-dependent live actions remain explicitly excluded unless authorized.
- [x] Fresh Windows and Linux setup works from documented commands with pinned compatible dependencies.
- [x] POM composition is acyclic, typed, and free of test-side selectors and generic legacy wrappers.
...
- [x] No implementation is labeled complete merely because a static badge or old checklist says so.
...
## Execution evidence - Phase 7 (local implementation)

2026-09-14, `improve/phase-7-ci`, parent `0c08363`. Tasks 7.3-7.5 implemented and locally verified; 7.3 stays unchecked until published workflow outcomes are verified. Hosted execution/settings remain pending. Three workflows use pinned Actions, bounded timeouts, target-specific summaries and 14-day artifacts. actionlint 1.7.12 passes; its downloaded checksum was verified. Summary rendering handles success/failure and exits 1 for missing JSON or JUnit. No workflow masks pytest failures.

Final local code passed 62 checks on Windows Chromium (14.58s) and clean Debian/Python 3.12 Docker containers with Chromium (18.38s), Firefox (29.79s), and WebKit (25.25s), no skips/retries. Windows Firefox/WebKit UI also passed 25 cases each. Windows Pixel 7 emulation passed four smoke cases (3.13s). A real live selector ambiguity was diagnosed and fixed: the product submit locator also matched an animation placeholder. The mock now contains that duplicate. Corrected live smoke passed three read-only cases (8.06s); first failing and corrected evidence remain in ignored reports/phase7-live-*. No cart or form submission occurred.

Tasks 7.1 and 7.2 remain open specifically for passing/failing remote PR runs, artifact inspection and the exact-SHA CI exit gate. Branch protection instructions are prepared in docs/ci-maintenance.md; settings have not been applied. Task 7.3 local work covers workflow preparation, safe selection and manual session execution; its published badge/schedule outcome remains open. Task 7.4 demonstrates local browser compatibility; hosted Ubuntu is pending. Phase 8 documentation can proceed independently while those external gates await publication; the core milestone is not yet met.

## Execution evidence - Phase 8

2026-09-14, `improve/phase-8-portfolio`, validated executable revision `68a09f442672f3a5cdbff6a46c277e90627ed429`. Tasks 8.1-8.6 complete as local handoff work. Rewrote the README with target-specific scope, architecture, actual workflow badges (publication pending), PowerShell/POSIX quickstarts and report instructions. Added the engineering case study, PR/defect templates, agent entry links, maintenance procedure and release-note draft. Preserved MIT source licensing and the original client's portfolio-permission statement as historical context; no expanded branding or transaction rights asserted.

Fresh local clones and new venvs installed the hashed lock, passed pip check and ran the documented setup: Windows Python 3.12.4, 62 passed in 19.78s; Debian/Python 3.12.14, 62 passed in 26.85s. Docker/nginx reuse passed 62 from Windows (16.75s) and 25 UI cases from Linux (14.71s). Linux Pixel 7 emulation passed four smoke cases (8.81s). One ad hoc nested-shell command initially split the device name and produced four setup errors; the corrected direct Docker invocation passed without any project-code change or automatic retry. The owned nginx validation container was stopped and removed; unrelated containers were preserved.

`docs/evidence/release-verification.json` contains 13 sanitized runs, exact revisions, summary-measured durations and JUnit count verification; earlier intentional/diagnosed failures are retained honestly. Raw reports/logs stay ignored. Black (34 files), Flake8, workflow validation and local documentation links pass. All executable files/pins remain identical to the clean validated code commit; subsequent handoff changes are documentation/templates only.

Core portfolio milestone: NOT YET MET solely because required remote CI acceptance has not run. Next ready work: publish the reviewed candidate when requested, follow docs/ci-maintenance.md for passing/failing PR artifact evidence (7.1/7.2), verify published live/compatibility outcomes (7.3), then apply/verify the documented required branch checks. No push, PR, merge, repository-setting change, tag or release was performed. Do not mark those gates complete from the local results above.

## Execution evidence - Hosted acceptance

2026-09-15 UTC / 2026-09-14 Buenos Aires, `improve/phase-8-portfolio`. Continuing the owner's request, published the prepared branch and opened [draft PR #1](https://github.com/MiltonKlun/PG_Original_POM/pull/1). Completed Tasks 7.1-7.3 against real GitHub runners; main was not merged or changed.

The [initial run](https://github.com/MiltonKlun/PG_Original_POM/actions/runs/34922276534) exposed an Ubuntu CI argument bug: dots in `quality-ubuntu-24.04` violated the report-ID contract. Commit `8b80f72` uses explicit safe matrix report IDs. The [corrected portfolio run](https://github.com/MiltonKlun/PG_Original_POM/actions/runs/34922388895) passed all three jobs: 37 offline on Ubuntu, 37 offline on Windows and 25 Chromium UI against Docker. Downloaded JUnit/JSON agree; standalone HTML opens without JS errors.

[Temporary draft PR #2](https://github.com/MiltonKlun/PG_Original_POM/pull/2) changed the expected shirt price by one cent. Its [deliberate failure run](https://github.com/MiltonKlun/PG_Original_POM/actions/runs/34922413481) failed exactly two cart assertions, while both quality jobs passed. The UI test step remained exit 1; summary, server-log collection/container cleanup and artifact upload succeeded. Downloaded evidence includes two valid trace ZIPs, two screenshots, two videos, server logs and matching HTML/JUnit/JSON (23 passed, 2 failed). The screenshot was reviewed and both passing/failing HTML reports rendered offline.

Restoring the original expectation produced [green recovery CI](https://github.com/MiltonKlun/PG_Original_POM/actions/runs/34922559438). [Hosted compatibility](https://github.com/MiltonKlun/PG_Original_POM/actions/runs/34922559490) passed Firefox 25, WebKit 25 and Pixel 7 Chromium emulation 4. [Hosted live smoke](https://github.com/MiltonKlun/PG_Original_POM/actions/runs/34922559492) passed 3, deselected 59, and performed no cart/form submission. Optional workflows used temporary PR triggers because GitHub requires workflow_dispatch definitions on main. Their runtime steps were unchanged. The triggers were reverted and PR #2 closed; no experiment change was merged.
