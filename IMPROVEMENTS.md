...
2026-09-14, `improve/phase-7-ci`, parent `0c08363`. Tasks 7.3-7.5 implemented and locally verified; hosted execution/settings remain pending. Three workflows use pinned Actions, bounded timeouts, target-specific summaries and 14-day artifacts. actionlint 1.7.12 passes; its downloaded checksum was verified. Summary rendering handles success/failure and exits 1 for missing JSON or JUnit. No workflow masks pytest failures.
...
Tasks 7.1 and 7.2 remain open specifically for passing/failing remote PR runs, artifact inspection and the exact-SHA CI exit gate. Branch protection instructions are prepared in docs/ci-maintenance.md; settings have not been applied. Task 7.3 completion covers workflow preparation, safe selection and manual session execution, not an already-published badge/schedule. Task 7.4 demonstrates local browser compatibility; hosted Ubuntu is pending. Phase 8 documentation can proceed independently while those external gates await publication; the core milestone is not yet met.
